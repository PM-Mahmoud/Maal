 ...
 * Insurance Coverage      10  Life + income protection + professional indemnity
 ...
 *
 * ─── Additional outputs ────────────────────────────────────────────────────
 * halalComplianceScore   0–100  % of portfolio in halal/ethical investments
 ...
 *
 * Health professionals face unique income interruption risks: malpractice
 * claims (no income during investigation), illness, injury. Score based on
 * presence of income protection + life/TPD + professional indemnity.
 *
 ...
 *   Professional Indemnity (3 pts): has it → 3, not sure → 1, no → 0
 *
...
// ─── Halal/Ethical Compliance Score ──────────────────────────────────────────

/**
 * halalComplianceScore — 0–100
 *
 * Proportion of non-super investment portfolio held in halal-compliant or
 * ethically-screened assets.
 *
 * User input: ethicalAllocation — JSON array:
 *   [{ assetClass: string, percentage: number, halal: boolean }]
 * e.g. [{"assetClass":"Ethical ETF","percentage":60,"halal":true},
 *       {"assetClass":"Conventional Shares","percentage":40,"halal":false}]
 *
 * Halalan-permissible asset classes (approximate list):
 *   Islamic equities (screened), gold, term deposits, shariah-compliant funds,
 *   green/renewable bonds, ethical ETFs (ESG-screened)
 *
 * Non-permissible: conventional shares (unscreened), gambling operators,
 *   alcohol producers, pork, riba-based instruments, crypto (speculative)
 *
 * Scoring: sum of percentages where halal=true, capped at 100
 */
function computeHalalComplianceScore(investmentAllocation) {
  ...
  // Map known halal/ethical and non-compliant asset classes
  const halalKeywords = ['islamic', 'halal', 'shariah', 'ethical', 'esg', 'green',
    'renewable', 'sustainable', 'clean energy', 'gold', 'commodity'];
  const nonHalalKeywords = ['crypto', 'gambling', 'conventional', 'alcohol', 'pork',
  ...
  arr.forEach((item) => {
    ...
    const isHalal = halalKeywords.some((k) => name.includes(k));
    const isNonHalal = nonHalalKeywords.some((k) => name.includes(k)) && !isHalal;

    if (item.halal === true || isHalal) halalPct += pct;
    else if (isNonHalal) halalPct += 0; // already 0
    else halalPct += pct * 0.5; // ambiguous → 50% credit
  });

  return Math.round(Math.min((halalPct / total) * 100, 100));
}
...
function buildDiagnosis(score, grade, data, components) {
  ...
  const prof = data.profession || 'health professional';
...
function buildRecommendations(data, components) {
  ...
  if (components.insuranceCoverage < 5 && insuranceCover !== 'full') {
    recs.push({
      ...
      detail: 'As a health professional, income protection is your most important financial asset — far more critical than investment returns. Aim for cover that replaces ≥ 75% of income for 2+ years, with a waiting period ≤ 30 days. Employer cover is often inadequate; a standalone policy fills the gap.',
    });
  }

  // Priority 3: Investment diversification
  if (components.investmentDiversity < 7) {
    const ethicalScore = computeHalalComplianceScore(investmentAllocation);
    const scoreLabel = ethicalScore >= 80 ? 'strong halal' : ethicalScore >= 50 ? 'mixed halal/ethical' : 'minimal halal';
    recs.push({
      impact: 3,
      title: 'Diversify your investment portfolio',
      detail: investmentBalance === 0
        ? `No investments detected outside super. Even $300/month into a diversified ETF (e.g. ASX:VAS, ASX:VGS, or a halal-screened fund) builds long-term wealth accessible before retirement. A simple 3–4 fund portfolio beats most actively managed funds after fees.`
        : `Your non-super portfolio has concentration risk or limited diversification. Spreading across 3–4 uncorrelated asset classes (Australian shares, international shares, defensive assets, halal-compliant options) reduces volatility without sacrificing long-term return.`,
    });
  }
