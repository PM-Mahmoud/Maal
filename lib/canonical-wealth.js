...
// Rows may arrive camelCase (in-app normalized) or snake_case (raw DB); read both
// for every field, matching the normalization used by ownershipShares/valuationFreshness.
const valSubjectType = (row) => row.subjectType ?? row.subject_type;
const valSubjectKey = (row) => row.subjectKey ?? row.subject_key;
const valAsOf = (row) => row.asOf ?? row.as_of;
const valRecordedAt = (row) => row.recordedAt ?? row.recorded_at ?? row.created_at ?? valAsOf(row);

function latestValuations(valuations = []) {
  const groups = new Map();
  for (const row of valuations) {
    const key = `${valSubjectType(row)}:${valSubjectKey(row)}:${row.classification}`;
  ...
  for (const rows of groups.values()) {
    ...
    result.push(heads.reduce((current, row) => {
      ...
      const rowTime = new Date(valAsOf(row)).getTime();
      const currentTime = new Date(valAsOf(current)).getTime();
      const rowRecorded = new Date(valRecordedAt(row)).getTime();
      const currentRecorded = new Date(valRecordedAt(current)).getTime();
...
function summarizeCanonicalAllocation(snapshot = {}) {
  ...
  for (const row of latestValuations(snapshot.valuations)) {
    ...
    if (!holding) continue;
    const currency = String(row.currency || 'AUD').toUpperCase();
    // Non-AUD rows only contribute when an AUD presentation amount is available;
    // never mix a foreign-currency minor amount into the AUD allocation total.
    const presentationCurrency = String(row.presentationCurrency || row.presentation_currency || currency).toUpperCase();
    const amountMinor = currency === 'AUD'
      ? row.amountMinor
      : (presentationCurrency === 'AUD' ? (row.presentationAmountMinor ?? row.presentation_amount_minor) : null);
