...
// reference) into the user's manual profile entries so the dashboard, Maal
...
// Classifies a Basiq institution_type + balance into one of the 4 asset/
// liability buckets a connected account can land in. Pure function, shared
// between aggregateConnected() below (legacy count/badge display) and
// routes/basiq.js's sync into the granular asset tables (db/assets.js).
function classifyAccountType(institutionType, balance) {
  const bal = Number(balance) || 0;
  const type = String(institutionType || '').toLowerCase();
  if (type.includes('credit') || type.includes('loan') || type.includes('mortgage') || bal < 0) return 'debt';
  if (type.includes('super')) return 'super';
  if (type.includes('invest') || type === 'broker') return 'invest';
  return 'cash'; // transaction / savings / term-deposit / unknown → cash
}
...
function aggregateConnected(accounts) {
  ...
  for (const a of accounts || []) {
    ...
    sums.count++;
    const bucket = classifyAccountType(a.institution_type, bal);
    if (bucket === 'debt') sums.debt += Math.abs(bal);
    else sums[bucket] += bal;
  }
  return sums;
}
...
module.exports = { classifyAccountType, aggregateConnected, buildEffectiveProfile };
