function detectCashRisks(forecast, options = {}) {
  ...
  const horizon = new Date(`${forecast.start_date}T00:00:00Z`); horizon.setUTCDate(horizon.getUTCDate() + Number(forecast.days || 0));
  const upcoming = (forecast.recurring || []).filter((item) => item.kind !== 'income' && item.nextEstimate && new Date(`${item.nextEstimate}T00:00:00Z`) <= horizon)
