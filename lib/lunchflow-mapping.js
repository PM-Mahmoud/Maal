...
function mapHolding(holding, accountId, observedAt = new Date().toISOString()) {
  const security = holding?.security || {};
  const isin = String(security.isin || '').trim().toUpperCase() || null;
  const figi = String(security.figi || '').trim().toUpperCase() || null;
  const ticker = String(security.tickerSymbol || '').trim().toUpperCase() || null;
  const exchange = String(security.exchange || '').trim().toUpperCase() || null;
  const name = String(security.name || ticker || isin || figi || 'Investment holding').trim().slice(0, 240);
  const currency = String(holding.currency || security.currency || 'AUD').slice(0, 3).toUpperCase();
  const holdingKey = isin
    ? `isin:${isin}`
    : figi
      ? `figi:${figi}`
      : ticker
        ? `ticker:${exchange || 'unknown'}:${ticker}:${currency}`
        : `name:${name.toLowerCase().replace(/\s+/g, ' ')}:${currency}`;
  return {
    account_reference: accountReference(accountId),
    ...
    exchange,
    currency,
    units: String(finiteNumber(holding.quantity)),
    price_minor: Math.round(finiteNumber(holding.price) * 100),
    value_minor: Math.round(finiteNumber(holding.value) * 100),
    cost_basis_minor: Math.round(finiteNumber(holding.costBasis) * 100),
    observed_at: observedAt,
  };
}

module.exports = { accountReference, classifyAccount, mapAccount, mapTransaction, mapHolding };
