...
const { getConstants, medicareLevy } = require('./au-constants');
...
// band rates on income above each boundary; a flat top rate of total once far
// enough above (all rates/thresholds sourced from lib/au-constants.js).
...
function estimateTax(profile) {
  ...
  const medicare = medicareLevy(income); // includes the low-income shading-in band
