...
const CONSTANT_SETS = {
  '2025-26': {
    ...
    medicare: {
      levyRate: 0.02,
      // Single, no dependants (2025-26, uplifted 2.9% retrospectively to 1 July
      // 2025). Below `lowIncomeSingle` = no levy; between it and
      // `lowIncomeSingleUpper` the levy shades in at 10c per $1 over the lower
      // threshold; above the upper threshold the full 2% applies. We model the
      // single-taxpayer case only (family thresholds not modelled).
      lowIncomeSingle: 28011,
      lowIncomeSingleUpper: 35013,
      phaseInRate: 0.10,
    },
    ...
    hecs: {
      ...
      topRate: 0.10, // flat 10% of total repayment income once above tenPercentAbove
  ...
  '2026-27': {
    ...
    medicare: {
      levyRate: 0.02,
      // 2026-27 low-income thresholds are indexed retrospectively in the
      // following budget and not yet legislated; carry the 2025-26 figures
      // forward until the ATO confirms them (the drift-check will flag any gap).
      lowIncomeSingle: 28011,
      lowIncomeSingleUpper: 35013,
      phaseInRate: 0.10,
    },
