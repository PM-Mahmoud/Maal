...
function calculateZakat(input) {
  ...
  const valuationIso = String(input.valuationDate);
  const valuationDate = new Date(`${valuationIso}T00:00:00Z`);
  // Reject invalid calendar dates that JS silently normalises (e.g. 2026-02-30 →
  // 2026-03-02): require the parsed UTC date to round-trip to the exact input.
  if (!/^\d{4}-\d{2}-\d{2}$/.test(valuationIso)
    || Number.isNaN(valuationDate.getTime())
    || valuationDate.toISOString().slice(0, 10) !== valuationIso) {
    throw new Error('A valid valuation date is required');
  }
