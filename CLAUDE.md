...
- **Transactions depth (2026-07-12, PR 6 of specs/silvia-parity-tier1-2.md)**: 18-group AU category taxonomy in `lib/transaction-categories.js` (+ keyword `autoCategorize()` fallback). Rules engine + subscriptions detection are PURE in `services/transaction-rules.js` (`matchRule`/`computeAssignments`/`detectSubscriptions` — deterministic-tested in `test/transaction-rules.test.js`). Categories are stored in a SEPARATE `transaction_categories` table (FK to transactions) so the **protected `transactions` table is never altered**; rules in `transaction_rules` (migration 1752600000000, both additive/FK-only). Endpoints (all before `/v1/:table`): GET `/v1/transaction-categories`, dedicated GET `/v1/transactions` (joins category + auto-category fallback for display), GET `/v1/transactions/subscriptions`, PATCH `/v1/transactions/:id/category`, GET/POST/DELETE `/v1/transaction-rules`, POST `/v1/transaction-rules/apply`. Client: transactions page All/Subscriptions toggle + `RulesModal.tsx` (create rules, apply-to-all). `db/transactions.js` gains the category/rules helpers
